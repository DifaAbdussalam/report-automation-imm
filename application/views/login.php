    ...
    <div class="login-box">
	  ...
	  <div class="login-logo">
		...
		if ($this->session->flashdata('error_login')){ 
			echo'<a href="'.site_url().'">'.$this->session->flashdata('error_login').'</a>';
		}else{
			echo'<a href="'.site_url().'">LOGIN ADMIN</a>';
		}
		?>
      </div>
		<div style="margin-left:25%;margin-right:25%;margin-bottom:20px;">
			<img src="<?php echo asset_url('core/img/logo.png');?>" class="img-responsive">
		</div>
		
		<form method="post" name="login" action="<?php echo site_url('login'); ?>">	
