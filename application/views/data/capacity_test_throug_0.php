...
<section class="content-header">
  <h1>Data capacity_test_throug_0</h1>
	<ol class="breadcrumb">
	<li><a href="#">Data</a></li>
		  ...
		  <table id="myTable" class="display table-bordered table-striped table-hover" cellspacing="0" width="100%">
			...
			if($mdata->num_rows()>0){
				...
				foreach($mdata->result() as $rows){
					...
					echo '<td>'.$rows->errorCode.'</td>';
					echo '<td>'.$rows->Direction.'</td>';
					echo '<td>'.$rows->DLThrpt.'</td>';
					echo '<td>'.$rows->ULThrpt.'</td>';
					echo '<td>'.$rows->URICount.'</td>';
					echo '<td>'.$rows->Success.'</td>';
					echo '<td>'.$rows->CapaDLOK.'</td>';
					echo '</tr>';
					$no++;
				}
			}
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="frm-import" method="post" class="form-horizontal" action="<?php echo site_url('capacity_test_throug_0/upload'); ?>" enctype="multipart/form-data">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
		...
		<div class="modal-body">
        <form id="frm-import" method="post" class="form-horizontal" action="<?php echo site_url('capacity_test_throug_0/export'); ?>" enctype="multipart/form-data">
			<div class="form-group">
...
$(document).ready(function() {
	...
	$("#kota").change(function(){
		...
		$.ajax({
			type: "POST",
			url : "<?php echo site_url('capacity_test_throug_0/filter_kota');?>",
			data: {kota:kota},
			beforeSend: function (){
				$(".modal").show();
			},
