...
class Http_browser_through_0 extends CI_Controller 
{
	...
	function index()
	{
		...
		$data['pages'] 		= 'data/http_browser_through_0';
		$data["date"]		= date('Y-m-d');
		$data["mkota"]		= $this->app_model->master("kota");
		$data["mdata"]		= $this->data_model->data_http_browser_through_0();
	...
	function upload()
	{
		if($this->input->post()){
			...
			if (isset($_FILES['userfile']) && $_FILES['userfile']['size'] > 0){
				...
				$config["file_name"]	=md5('dmyhis').'_upload_param7_'.$_FILES['userfile']['name'];
				...
				if($this->upload->do_upload()){
					...
					foreach ($sheet as $row){
						if($numrow > 1){
							...
							$this->app_model->simpan("http_browser_through_0",$data);
						}
						$numrow++;
					}
					unlink($file_path);
					$this->session->set_flashdata('info','Data berhasil diupload !');
					redirect('http_browser_through_0');
				}else{
	...
	function export()
	{
		if($this->input->post()){
			...
			$this->excel->getActiveSheet()->setTitle('http_browser_through_0');
			...
			$this->excel->getActiveSheet()->getCell('AK1')->setValue("inputDate");
			$this->excel->getActiveSheet()->getCell('AL1')->setValue("Kota");
			
			$this->excel->getActiveSheet()->getStyle('A1:AL1')->getFont()->setBold(true);
			$this->excel->getActiveSheet()->getStyle('A1:AL1')->getFont()->setSize(12);
			$this->excel->getActiveSheet()->getStyle('A1:AL1')
										  ->getAlignment()
										  ->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		
			$no=2;
			$index=0;
			$mreport=$this->data_model->export_http_browser_through_0($start,$end,$kota);
			...
			$this->excel->getActiveSheet()->getStyle("A1:AL".($no))->applyFromArray($borderArray, False);
			
			$filename="export_http_browser_through_0_".$kota."_".$start."-SD-".$end.".xls";
