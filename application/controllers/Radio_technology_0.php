...
class Radio_technology_0 extends CI_Controller 
{
	...
	function index()
	{
		...
		$data['pages'] 		= 'data/radio_technology_0';
		$data["date"]		= date('Y-m-d');
		$data["mkota"]		= $this->app_model->master("kota");
		$data["mdata"]		= $this->data_model->data_radio_technology_0();
	...
	function upload()
	{
		if($this->input->post()){
			...
			if (isset($_FILES['userfile']) && $_FILES['userfile']['size'] > 0){
				...
				$config["file_name"]	=md5('dmyhis').'_upload_param9_'.$_FILES['userfile']['name'];
				...
				if($this->upload->do_upload()){
					...
					foreach ($sheet as $row){
						if($numrow > 1){
							...
							$this->app_model->simpan("radio_technology_0",$data);
						}
						$numrow++;
					}
					unlink($file_path);
					$this->session->set_flashdata('info','Data berhasil diupload !');
					redirect('radio_technology_0');
				}else{
	...
	function export()
	{
		if($this->input->post()){
			...
			$this->excel->getActiveSheet()->setTitle('radio_technology_0');
			...
			$this->excel->getActiveSheet()->getCell('AJ1')->setValue("Technology");
			...
			$no=2;
			$index=0;
			$mreport=$this->data_model->export_radio_technology_0($start,$end,$kota);
			...
			$filename="export_radio_technology_0_".$kota."_".$start."-SD-".$end.".xls";
