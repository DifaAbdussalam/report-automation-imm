...
class Ping_rtt_ms_0 extends CI_Controller 
{
	...
	function index()
	{
		...
		$data['pages'] 		= 'data/ping_rtt_ms_0';
		$data["date"]		= date('Y-m-d');
		$data["mkota"]		= $this->app_model->master("kota");
		$data["mdata"]		= $this->data_model->data_ping_rtt_ms_0();
	...
	function upload()
	{
		if($this->input->post()){
			...
			if (isset($_FILES['userfile']) && $_FILES['userfile']['size'] > 0){
				...
				$config["file_name"]	=md5('dmyhis').'_upload_param8_'.$_FILES['userfile']['name'];
				...
				if($this->upload->do_upload()){
					...
					foreach ($sheet as $row){
						if($numrow > 1){
							...
							$data['Host'] 			= $row['AL'];
							$this->app_model->simpan("ping_rtt_ms_0",$data);
						}
						$numrow++;
					}
					unlink($file_path);
					$this->session->set_flashdata('info','Data berhasil diupload !');
					redirect('ping_rtt_ms_0');
				}else{
	...
	function export()
	{
		if($this->input->post()){
			...
			$this->excel->getActiveSheet()->setTitle('ping_rtt_ms_0');
			...
			$this->excel->getActiveSheet()->getCell('AJ1')->setValue("RTT");
			...
			$no=2;
			$index=0;
			$mreport=$this->data_model->export_ping_rtt_ms_0($start,$end,$kota);
			...
			$filename="export_ping_rtt_ms_0_".$kota."_".$start."-SD-".$end.".xls";
