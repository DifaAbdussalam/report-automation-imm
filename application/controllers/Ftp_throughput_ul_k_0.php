...
class Ftp_throughput_ul_k_0 extends CI_Controller 
{
	...
	function index()
	{
		...
		$data['pages'] 		= 'data/ftp_throughput_ul_k_0';
		$data["date"]		= date('Y-m-d');
		$data["mkota"]		= $this->app_model->master("kota");
		$data["mdata"]		= $this->data_model->data_ftp_throughput_ul_k_0();
	...
	function upload()
	{
		if($this->input->post()){
			...
			if (isset($_FILES['userfile']) && $_FILES['userfile']['size'] > 0){
				...
				$config["file_name"]	=md5('dmyhis').'_upload_param6_'.$_FILES['userfile']['name'];
				...
				if($this->upload->do_upload()){
					...
					foreach ($sheet as $row){
						if($numrow > 1){
							...
							$data['operation'] 		= $row['AK'];
							$data['errorcode'] 		= $row['AL'];
							$this->app_model->simpan("ftp_throughput_ul_k_0",$data);
						}
						$numrow++;
					}
					unlink($file_path);
					$this->session->set_flashdata('info','Data berhasil diupload !');
					redirect('ftp_throughput_ul_k_0');
				}else{
	...
	function export()
	{
		if($this->input->post()){
			...
			$this->excel->getActiveSheet()->setTitle('ftp_throughput_ul_k_0');
			...
			$this->excel->getActiveSheet()->getCell('AK1')->setValue("operation");
			$this->excel->getActiveSheet()->getCell('AL1')->setValue("errorcode");
			...
			$no=2;
			$index=0;
			$mreport=$this->data_model->export_ftp_throughput_ul_k_0($start,$end,$kota);
			...
			$filename="export_ftp_throughput_ul_k_0_".$kota."_".$start."-SD-".$end.".xls";
