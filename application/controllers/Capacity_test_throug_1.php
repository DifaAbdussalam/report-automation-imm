...
class Capacity_test_throug_1 extends CI_Controller 
{
	...
	function index()
	{
		...
		$data['pages'] 		= 'data/capacity_test_throug_1';
		$data["date"]		= date('Y-m-d');
		$data["mkota"]		= $this->app_model->master("kota");
		$data["mdata"]		= $this->data_model->data_capacity_test_throug_1();
	...
	function upload()
	{
		if($this->input->post()){
			...
			if (isset($_FILES['userfile']) && $_FILES['userfile']['size'] > 0){
				...
				$config["file_name"]	=md5('dmyhis').'_upload_param4_'.$_FILES['userfile']['name'];
				...
				if($this->upload->do_upload()){
					...
					foreach ($sheet as $row){
						if($numrow > 1){
							...
							$data['errorCode'] 		= $row['AJ'];
							$data['Direction'] 		= $row['AK'];
							$data['DLThrpt'] 		= $row['AL'];
							$data['ULThrpt'] 		= $row['AM'];
							$data['URICount'] 		= $row['AN'];
							$data['Success'] 		= $row['AO'];
							$data['CapaULOK'] 		= $row['AP'];
							$this->app_model->simpan("capacity_test_throug_1",$data);
						}
						$numrow++;
					}
					unlink($file_path);
					$this->session->set_flashdata('info','Data berhasil diupload !');
					redirect('capacity_test_throug_1');
				}else{
	...
	function export()
	{
		if($this->input->post()){
			...
			$this->excel->getActiveSheet()->setTitle('capacity_test_throug_1');
			...
			$this->excel->getActiveSheet()->getCell('AJ1')->setValue("errorCode");
			$this->excel->getActiveSheet()->getCell('AK1')->setValue("Direction");
			$this->excel->getActiveSheet()->getCell('AL1')->setValue("DLThrpt");
			$this->excel->getActiveSheet()->getCell('AM1')->setValue("ULThrpt");
			$this->excel->getActiveSheet()->getCell('AN1')->setValue("URICount");
			$this->excel->getActiveSheet()->getCell('AO1')->setValue("Success");
			$this->excel->getActiveSheet()->getCell('AP1')->setValue("CapaULOK");
			$this->excel->getActiveSheet()->getCell('AQ1')->setValue("inputDate");
			$this->excel->getActiveSheet()->getCell('AR1')->setValue("Kota");
			$this->excel->getActiveSheet()->getStyle('A1:AR1')->getFont()->setBold(true);
			$this->excel->getActiveSheet()->getStyle('A1:AR1')->getFont()->setSize(12);
			$this->excel->getActiveSheet()->getStyle('A1:AR1')
										  ->getAlignment()
										  ->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		
			$no=2;
			$index=0;
			$mreport=$this->data_model->export_capacity_test_throug_1($start,$end,$kota);
			...
			$this->excel->getActiveSheet()->getStyle("A1:AR".($no))->applyFromArray($borderArray, False);
			
			$filename="export_capacity_test_throug_1_".$kota."_".$start."-SD-".$end.".xls";
