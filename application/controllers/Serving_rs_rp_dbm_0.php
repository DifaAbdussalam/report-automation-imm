...
class Serving_rs_rp_dbm_0 extends CI_Controller 
{
	...
	function index()
	{
		...
		$data['pages'] 		= 'data/serving_rs_rp_dbm_0';
		$data["date"]		= date('Y-m-d');
		$data["mkota"]		= $this->app_model->master("kota");
		$data["mdata"]		= $this->data_model->data_serving_rs_rp_dbm_0();
	...
	function upload()
	{
		if($this->input->post()){
			...
			if (isset($_FILES['userfile']) && $_FILES['userfile']['size'] > 0 && $_FILES['userfile']['error'] == 0){
				...
				$config["file_name"]	=md5('dmyhis').'_upload_param12_'.$_FILES['userfile']['name'];
				...
				if($this->upload->do_upload()){
					...
					foreach ($sheet as $row){
						if($numrow > 1){
							...
							$data['RSRP'] 			= $row['AJ'];
							$data['NumMeasuredCells']= $row['AK'];
							$data['ChnPCI'] 		= $row['AL'];
							$this->app_model->simpan("serving_rs_rp_dbm_0",$data);
						}
						$numrow++;
					}
					unlink($file_path);
					$this->session->set_flashdata('info','Data berhasil diupload !');
					redirect('serving_rs_rp_dbm_0');
				}else{
	...
	function export()
	{
		if($this->input->post()){
			...
			$this->excel->getActiveSheet()->setTitle('serving_rs_rp_dbm_0');
			...
			$this->excel->getActiveSheet()->getCell('AJ1')->setValue("RSRP");
			$this->excel->getActiveSheet()->getCell('AK1')->setValue("NumMeasuredCells");
			$this->excel->getActiveSheet()->getCell('AL1')->setValue("ChnPCI");
			...
			$no=2;
			$index=0;
			$mreport=$this->data_model->export_serving_rs_rp_dbm_0($start,$end,$kota);
			...
			$filename="export_serving_rs_rp_dbm_0_".$kota."_".$start."-SD-".$end.".xls";
