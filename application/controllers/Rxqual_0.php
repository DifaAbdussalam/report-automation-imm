...
class Rxqual_0 extends CI_Controller 
{
	...
	function index()
	{
		...
		$data['pages'] 		= 'data/rxqual_0';
		$data["date"]		= date('Y-m-d');
		$data["mkota"]		= $this->app_model->master("kota");
		$data["mdata"]		= $this->data_model->data_rxqual_0();
	...
	function upload()
	{
		if($this->input->post()){
			...
			if (isset($_FILES['userfile']) && $_FILES['userfile']['size'] > 0){
				...
				$config["file_name"]	=md5('dmyhis').'_upload_param11_'.$_FILES['userfile']['name'];
				...
				if($this->upload->do_upload()){
					...
					foreach ($sheet as $row){
						if($numrow > 1){
							...
							$data['RxQual'] 		= $row['AJ'];
							$data['RxQualVal'] 		= $row['AK'];
							$data['ChNr'] 			= $row['AL'];
							$this->app_model->simpan("rxqual_0",$data);
						}
						$numrow++;
					}
					unlink($file_path);
	...
	function export()
	{
		if($this->input->post()){
			...
			$this->excel->getActiveSheet()->setTitle('rxqual_0');
			...
			$this->excel->getActiveSheet()->getCell('AJ1')->setValue("RxQual");
			$this->excel->getActiveSheet()->getCell('AK1')->setValue("RxQualVal");
			...
			$no=2;
			$index=0;
			$mreport=$this->data_model->export_rxqual_0($start,$end,$kota);
			...
			$filename="export_rxqual_0_".$kota."_".$start."-SD-".$end.".xls";
