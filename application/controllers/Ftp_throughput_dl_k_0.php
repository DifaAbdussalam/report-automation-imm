...
class Ftp_throughput_dl_k_0 extends CI_Controller 
{
	...
	function index()
	{
		...
		$data['pages'] 		= 'data/ftp_throughput_dl_k_0';
		$data["date"]		= date('Y-m-d');
		$data["mkota"]		= $this->app_model->master("kota");
		$data["mdata"]		= $this->data_model->data_ftp_throughput_dl_k_0();
	...
	function upload()
	{
		if($this->input->post()){
			...
			if (isset($_FILES['userfile']) && $_FILES['userfile']['size'] > 0){
				...
				$config["file_name"]	=md5('dmyhis').'_upload_param5_'.$_FILES['userfile']['name'];
				...
				if($this->upload->do_upload()){
					...
					foreach ($sheet as $row){
						if($numrow > 1){
							...
							$data['Throughput'] 	= $row['AJ'];
							$data['errorcode'] 		= $row['AK'];
							$data['host'] 			= $row['AL'];
							$data['operation'] 		= $row['AM'];
							$data['filename'] 		= $row['AN'];
							$this->app_model->simpan("ftp_throughput_dl_k_0",$data);
						}
						$numrow++;
					}
					unlink($file_path);
					$this->session->set_flashdata('info','Data berhasil diupload !');
					redirect('ftp_throughput_dl_k_0');
				}else{
	...
	function export()
	{
		if($this->input->post()){
			...
			$this->excel->getActiveSheet()->setTitle('ftp_throughput_dl_k_0');
			...
			$this->excel->getActiveSheet()->getCell('AJ1')->setValue("Throughput");
			$this->excel->getActiveSheet()->getCell('AK1')->setValue("errorcode");
			$this->excel->getActiveSheet()->getCell('AL1')->setValue("host");
			$this->excel->getActiveSheet()->getCell('AM1')->setValue("operation");
			$this->excel->getActiveSheet()->getCell('AN1')->setValue("filename");
			$this->excel->getActiveSheet()->getCell('AO1')->setValue("inputDate");
			$this->excel->getActiveSheet()->getCell('AP1')->setValue("Kota");
			 			

			$this->excel->getActiveSheet()->getStyle('A1:AP1')->getFont()->setBold(true);
			$this->excel->getActiveSheet()->getStyle('A1:AP1')->getFont()->setSize(12);
			$this->excel->getActiveSheet()->getStyle('A1:AP1')
										  ->getAlignment()
			...
			$this->excel->getActiveSheet()->getStyle("A1:AP".($no))->applyFromArray($borderArray, False);
			
			$filename="export_ftp_throughput_dl_k_0_".$kota."_".$start."-SD-".$end.".xls";
